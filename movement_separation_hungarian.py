...
# Attempting frame tracking & labelling using a combination of metrics and the Hungarian assignment algorithm
# Hungarian is only applied for frames with 3 detections
...
new_folder_dir = "/mnt/c/3HYPER FREEPLAY DV METRABS/Processed Data/Test"
...
folder_name = "3HYPER.0" + str(subject_num) + " FREEPLAY DV EXTRACTED"
...
if not os.path.exists(folder_path):
   ...
   # In case if 3 detections are found in the first frame 
    ...
    for frame in sorted(os.listdir(folder_path)[ref_frame_count+1:]):
        ...
        # Make IDs consistent (person_id should be type int, not list) 
        ...
        padded_frame = frame.zfill(4)
        # Save the separated IDs in a new folder for processed data
        new_file_name = os.path.join(new_folder_dir, folder_name, frame.replace(".json", ".json"))
